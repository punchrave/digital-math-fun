...
import { Eye, ZoomIn, ZoomOut, Contrast, Type, RotateCcw } from "lucide-react";
    ...
    <Sheet>
      <SheetTrigger asChild>
        <Button
          variant="outline"
          size="icon"
          className="fixed top-4 right-4 z-50 bg-background border-primary hover:bg-primary hover:text-primary-foreground"
      ...
      <SheetContent className="w-[320px] sm:w-[400px]">
        ...
        <div className="mt-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <Label htmlFor="high-contrast" className="text-base font-medium flex items-center gap-2 cursor-pointer">
          ...
          {/* Underline Links */}
          <div className="flex items-center justify-between">
            <Label htmlFor="underline-links" className="text-base font-medium cursor-pointer">
              Подчёркивать ссылки
            </Label>
            <Switch
              id="underline-links"
              checked={settings.underlineLinks}
              onCheckedChange={(checked) => setSettings(prev => ({ ...prev, underlineLinks: checked }))}
          ...
          {/* Large Spacing */}
          <div className="flex items-center justify-between">
            <Label htmlFor="large-spacing" className="text-base font-medium cursor-pointer">
              Увеличенные интервалы
            </Label>
            <Switch
              id="large-spacing"
              checked={settings.largeSpacing}
              onCheckedChange={(checked) => setSettings(prev => ({ ...prev, largeSpacing: checked }))}
